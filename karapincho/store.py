...
class Store:
    ...
    def create(self, source_type, source, title, job_id=None, lyric_settings=None):
        job_id = job_id or uuid.uuid4().hex
        now = time.time()
        with self.connect() as db:
            db.execute(
                """INSERT INTO jobs
                (id,created,updated,status,stage,title,source,source_type,elapsed_seconds,lyric_settings)
                VALUES (?,?,?,'queued','acquire',?,?,?,0,?)""",
                (job_id, now, now, title, source, source_type, json.dumps(lyric_settings or {}, ensure_ascii=False)),
            )
    ...
    def rebuild(self, job_id, settings):
        with self.connect() as db:
            db.execute("BEGIN IMMEDIATE")
            ...
            db.execute("UPDATE jobs SET lyric_settings=?, status='queued', cancel_requested=0, "
                       "error=NULL, warnings='[]', progress=0, lyric_source=NULL, updated=? WHERE id=?",
